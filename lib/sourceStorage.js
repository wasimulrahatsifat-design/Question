...
export async function saveSource({
  ...
  if (supabase) {
    try {
      let filePath = null;
      let publicUrl = null;
      let sizeBytes = file ? file.size : new Blob([textContent]).size;
      ...
      if (file && (type === 'pdf' || type === 'image')) {
        ...
        // Clean ASCII storage path to prevent S3 UTF-8 header errors
        ...
        if (uploadError) {
          console.warn('Supabase bucket upload failed:', uploadError.message);
          throw new Error(`ক্লাউড স্টোরেজ আপলোড ত্রুটি: ${uploadError.message}`);
        }
      ...
      if (dbError && (dbError.message?.includes('chapters') || dbError.details?.includes('chapters') || dbError.code === '42703')) {
        console.warn('Retrying Supabase insert without chapters column for legacy schema...');
      ...
      if (dbError) {
        console.warn('Supabase DB insert failed:', dbError.message);
        throw new Error(`ডাটাবেজ টেবিল ত্রুটি: ${dbError.message}`);
      }
      ...
      console.warn('Supabase save failed, saving local copy:', supabaseErr);
      supabaseErrorMsg = supabaseErr.message || 'Supabase save failed';
    }
  }

  // Always save a copy to local IndexedDB to guarantee persistence
  try {
    ...
    const localRecord = {
      ...
      fileName: file ? file.name : null,
      blob: file || null,
      filePath: supabaseRecord?.filePath || null,
      publicUrl: supabaseRecord?.publicUrl || null,
      textContent: type === 'text' ? textContent : null,
      pageCount: pageCount || 1,
      size: file ? file.size : new Blob([textContent]).size,
      mimeType: file ? file.type : (type === 'pdf' ? 'application/pdf' : 'text/plain'),
      chapters: safeChapters,
      storageType: supabaseSuccess ? 'supabase' : 'indexeddb',
    ...
    if (supabaseSuccess && supabaseRecord) {
    ...
    throw new Error(supabaseErrorMsg || localErr.message || 'সোর্স সংরক্ষণ ব্যর্থ হয়েছে');
  }
}
