...
export async function processSelectedSources(selectedItems, onProgress = () => {}) {
  ...
  for (let i = 0; i < selectedItems.length; i++) {
    ...
    if (source.type === 'text') {
      ...
      try {
        if (Array.isArray(source.images) && source.images.length > 1) {
          // Multi-Image Folder Source: Process each image in the collection
          const totalImgs = source.images.length;
          const selectedImageIds = item.selectedImageIds;

          for (let imgIdx = 0; imgIdx < totalImgs; imgIdx++) {
            const imgMeta = source.images[imgIdx];
            const imgId = imgMeta.id || `${source.id}_img_${imgIdx + 1}`;

            // If selectedImageIds is specified and not empty, skip unselected images
            if (Array.isArray(selectedImageIds) && selectedImageIds.length > 0) {
              if (
                !selectedImageIds.includes(imgId) &&
                !selectedImageIds.includes(imgMeta.id) &&
                !selectedImageIds.includes(String(imgIdx + 1))
              ) {
                continue;
              }
            }

            onProgress(`ছবি প্রসেস করা হচ্ছে (${imgIdx + 1}/${totalImgs}): "${source.title}"...`);
            try {
              if (imgMeta.url) {
                const res = await fetch(imgMeta.url);
                if (res.ok) {
                  const blob = await res.blob();
                  const imgObj = await convertImageFileToBase64(blob);
                  images.push({
                    ...imgObj,
                    sourceId: imgId,
                    sourceTitle: `${source.title} (ছবি ${imgIdx + 1}/${totalImgs} - ${imgMeta.title || imgMeta.name || ''})`,
                  });
                }
              }
            } catch (imgErr) {
              console.warn(`Error processing image ${imgIdx + 1} of folder "${source.title}":`, imgErr);
            }
          }
        } else {
          // Single Image Source
