  ...
  sections.forEach((section, sIndex) => {
    ...
    docChildren.push(
      new Paragraph({
        ...
        children: [
          new TextRun({ 
            text: `${sectionNumStr} ${section.title}`, 
            bold: true, 
            font: FONT_RUN, 
            size: 24 // 12pt
          }),
    ...
    const isTfSection = section.id.includes('tf') || section.title.includes('সত্য/মিথ্যা') || section.title.includes('সত্য অথবা মিথ্যা');
    const isMcq = section.id.includes('mcq') || section.title.includes('সঠিক উত্তর') || (section.questions?.[0]?.options?.length > 0);

    // 7. ORAL SECTION: No questions, just heading and marks
    if (isOralSection) {
      return;
    }

    // 5. MATCHING SECTION (The Only Boxed Table)
    if (isMatchSection && section.questions && section.questions.length > 0) {
      ...
      // 1, 2, 3, 4, 6 Standard Question Types: All Size 12pt (24), ZERO SPACING, ZERO INDENT
      (section.questions || []).forEach((q, qIndex) => {
        ...
        // 3. Fill in Blanks in Answer Key
        ...
        // 1. MCQ Options: ক. অপশন ১     খ. অপশন ২ (Size 12pt = 24, Spacing 0)
        ...
        // 2 & 6. Short & Long Descriptive Answers in Answer Key (Size 12pt = 24, Spacing 0)
